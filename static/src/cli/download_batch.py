...
def download_list_parallel(acc_list, output_dir="./downloads", fmt='nifti', modality=None,
    ...
    task_download_queue = Queue()    # (task_id, accession, options)
            ...
            try:
                ...
                for attempt in range(3):
                    try:
                        response = requests.post(api_single, json=payload, timeout=request_timeout)
                        if response.status_code == 503:
                            tqdm.write(f"[!] 服务器队列已满: {accession}")
                            if attempt < 2:
                                time.sleep(10)
                                continue
                            break
                        if response.status_code != 200:
                            tqdm.write(f"[!] 提交失败 {accession} ({response.status_code}): {response.text}")
                            if attempt < 2:
                                time.sleep(2)
                                continue
                            break
